...
hl.bind(mainMod .. " + F", function()
    ...
    if w.floating then
        _float_saved[addr] = { x = w.at.x, y = w.at.y, sx = w.size.x, sy = w.size.y }
        ...
        local py = w.at.y
        hl.dispatch(hl.dsp.window.float({ action = "toggle" }))
        local saved = _float_saved[addr]
        if saved then
            sx = saved.sx
            sy = saved.sy
            px = saved.x
            py = saved.y
            _float_saved[addr] = nil
        end
        hl.dispatch(hl.dsp.window.resize({ x = sx, y = sy, relative = false }))
